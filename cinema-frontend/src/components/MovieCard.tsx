...
export default function MovieCard({ movie, selectedTime }: MovieCardProps) {
    ...
    return (
        <div>
            <div className="card shadow-lg bg-base-100 card-border overflow-hidden ">
                {/* Poster + Title */}
                ...
                <div className={"card-body text-center justify-center px-2"}>
                    <Link href={`/movies/${movie.id}`} className="card-title text-center justify-center hover:underline hover:text-secondary transition">
                        {movie.title}
                    </Link>
